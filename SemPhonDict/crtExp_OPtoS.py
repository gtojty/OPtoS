...
#       ortho: 8-letter length (3(con) + 2(vow) + 3(con))
...
# P to S:
# for creating example files: 
#       phon: 7-phoneme length (3(con) + 1(vow) + 3(con))
#       sem: 50/100/200/300 semantic items              
# ---------------------------------------------- #
# Based on Harm (1998)
...
for numSemItem in SemItemList:
    ...
    # write to example file
    writeExp(wordDF, './Tr_' + str(numSemItem) + '_PtoS_log.txt', 'log_freq', 7, "Phono", 0, 6, "Sem", 4, 6, PhonDic, SemDic)
    writeExp(wordDF, './Tr_' + str(numSemItem) + '_PtoS_sqrt.txt', 'sqrt_freq', 7, "Phono", 0, 6, "Sem", 4, 6, PhonDic, SemDic)
    # based on different lambda values of boxcox transformation
    temp = range(0,11,1)
    bcLambda = [i/10.0 for i in temp]
    for ll in bcLambda:
        colname = 'bcLambda_' + str(ll)
        writeExp(wordDF, './Tr_' + str(numSemItem) + '_PtoS_' + str(ll) + '.txt', colname, 7, "Phono", 0, 6, "Sem", 4, 6, PhonDic, SemDic)
